...
func getFileHandler(db *DB, q *queue) echo.HandlerFunc {
	return func(c echo.Context) error {
		fieID := c.Param("id")

		file, err := db.getFile(fieID)
		if err != nil {
			log.Error(err)
			return echo.NewHTTPError(http.StatusInternalServerError, "could not get file")
		}

		c.Response().Header().Set(
			echo.HeaderContentDisposition,
			fmt.Sprintf("attachment; filename=%s.csv", file.FileName),
		)
		return c.Blob(http.StatusOK, "text/csv", file.File)
	}
}
